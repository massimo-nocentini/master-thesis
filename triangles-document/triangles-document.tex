...
\begin{document}
    ...
    to fullfil the very first column, together with $d_{00}$, the root element), 
    ...
    We learnt this trick from Shapiro introductory article \emph{The Riordan group}.
    ...
    Two quick checks: in order to find $d_{7,5}$, choosen at random, 
    ...
    \end{displaymath}
    in order to find $d_{6,3}$, choosen at random, expand function $\Gamma$ with $k=3$:
    \begin{displaymath}
    ...
    Previous applications shows a pattern that can be pointed out looking at the generic device
    for a Riordan array $\mathcal{R}$ where function $h$ is its second component:
    \begin{displaymath}
        \left[y^{k} = \hat{h}(y) \Gamma(y) \big| y = h(t) \right]
    \end{displaymath}
    On the left hand side of the variable substitution writing there's an
    equation, where function $\Gamma$ is the unknown. In this format, respect
    an element $d_{nk}\in\mathcal{R}$, a constraint is stated, which read as follows: 
    to localize a sequence $\lbrace \gamma_{i} \rbrace_{i\in\mathbb{N}}$
    respect column $k$, combining all elements lying on the previous row $n-1$.

    Since we're dealing with an equation, we can augment it as desired in order to
    constraint over additional facts. For instance, consider the following question:
    find a sequence $\lbrace \gamma_{i} \rbrace_{i\in\mathbb{N}}$ such that 
    ...
    elements, define by a sequence $\lbrace \theta_{i} \rbrace_{i\in\mathbb{N}}$, 
    ...
    \begin{displaymath}
        \left[y^{k} = y^{c_\mu}\hat{h}(y)^{\alpha} \Gamma(y) + y^{c_\nu}\hat{h}(y)^{\beta} \Theta(y) \big| y = h(t) \right]
    \end{displaymath}
    ...
    \begin{split}
        ...
        &= -\frac{y^{3} + {\left(y^{2} - 1\right)} y^{k} + 6 \, y^{2} - {\left({\left(y - 1\right)} y^{k} + y^{2} + 3 \, y + 1\right)} \sqrt{y^{2} + 6 \, y + 1} + 6 \, y + 1}{2 \, {\left(y - 1\right)}}\\
    \end{split}
    \end{displaymath}
    in order to find $d_{8,1}$, choosen at random, expand function $\Gamma$ with $k=1$:
    \begin{displaymath}
    ...
    therefore $d_{8,1}=d_{9,2} -3\,d_{9,3} +11\,d_{9,4}-47\,d_{9,5} 
        +211\,d_{9,6} -987\,d_{9,7} +4747\,d_{9,8}-23335\,d_{9,9}$, 
        instantiating $15 = 113 -3\cdot377 +11\cdot681 -47\cdot681 +211\cdot377
            -987\cdot113 +4747\cdot17 -23335 + \epsilon = -154 + \epsilon$, 
            which holds, where
            $\epsilon = d_{6,0}+d_{6,1}+d_{6,2}+d_{6,3}+d_{6,4}+d_{6,5}+d_{6,6} = 

...
\begin{document}
    ...
    \added[id=dm]{A possible recurrence should be the following, inspired by the 
    computation of the \emph{fractal dimensionality} of Pascal array:}
    \begin{displaymath}
        \added[id=dm]{\mathcal{M}(h+2) = 2\,\left(\mathcal{M}(h+1) + \mathcal{M}(h)\right)}
    \end{displaymath}
    ...
    \begin{itemize}
        \item $\mathcal{M}_{h}$ repeats two times ``over the same columns'';
        \item $\mathcal{M}_{h+1}$ repeats two times: the first one covers the
        ...
        \item two upside-down triangles (ie. pointing downward), of coefficients
        ...
        \item another two triangles, with mixed coefficients, some of them

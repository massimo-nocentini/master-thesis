...
\begin{document}
    ...
    defined as:
    \begin{displaymath} 
        \mathcal{P} = \left(\frac{1}{1-t-t^2}, \frac{1-\sqrt{1-4t}}{2}  \right)
    \end{displaymath} 
    computing the compositional inverse of $h$ yields:
    \begin{displaymath} 
        \hat{h}(y) = y - y^2
    ...
    \begin{displaymath} 
        \begin{split} 
            & \mathcal{P}_{y=h(t)-1}\left( \frac{1}{1+y-2y^3-y^4}, 1+y \right) = \\
             &= \mathcal{P}_{h(t)}\left( \frac{1}{h(t)-2(h(t)-1)^3-(h(t)-1)^4}, h(t) \right)\\
             &= \mathcal{P}_{h(t)}\left( \frac{1}{1-h(t)+2h(t)^3-h(t)^4}, h(t) \right)\\
        \end{split} 
    \end{displaymath} 
    Now study the generating function which carries coefficients lying on column $k$
    of $\mathcal{P}_{h(t)}$:

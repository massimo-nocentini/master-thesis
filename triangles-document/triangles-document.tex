...
\begin{document}
    ...
    It's interesting to observe that function $\Gamma$ above, an algebric one,
    ...
    \begin{displaymath}
        \left.\left[
            ...
            \right| y = h(t) \right]
    \end{displaymath}
    a simple simplification rewrites:
    \begin{displaymath}
        \left.\left[
    ...
    \begin{itemize}
        ...
        \item it provides a ``factorization'' of function $A(t)$ with coefficients
            ...
            For the sake of clarity, consider Pascal array $\mathcal{P}$: recall that
            $\hat{h}(y)=\frac{y}{1+y}$ and $A(y)=1+y$ therefore, by \emph{division
            theorem}, there exist polynomials $\Omega_{0}$ and $\Omega_{1}$ such that:
            \begin{displaymath}
                \left.\left[
                    (1+y)^2 =  (1+y)\Omega_{0}(y) + y\,\Omega_{1}(y) 
                        \right| y = h(t) \right]
            \end{displaymath}
            dividing $(1+y)^2$ by $y$ yield $\Omega_{1}(y)=2+y$ and 
            $(1+y)\Omega_{0}(y)=1$. Putting it all together,
            the factorization of polynomial $A$ respect polynomial $\hat{h}$ is:
            \begin{displaymath}
                \left.\left[
                    A(y) =  \frac{1}{1+y} + \hat{h}(y)(2+y) \right| y = h(t) \right]
            \end{displaymath}

            If terms are not in a polynomial ring, we've difficulty to show a
            factorization: both Catalan array $\mathcal{C}$ both Delannoy array
        ...
        \item looking at the formulation under study, we observe that functions
            ...
            $\Omega_{0}(y)=\frac{1}{1+y}$, which says to sum elements on row $n-1$
            using alternating sings and, secondly, $\Omega_{1}(y)=2+y$, 
            which says to double the ``first'' element and sum to the ``second'' element
            on row $n-1$, all respect column index $k$. 
            
            Element $d_{7,4}$, choosen at random, is the combination 
            $d_{6,3}-d_{6,4}+d_{6,5}-d_{6,6}+2d_{5,3}+d_{5,4}$, instantiating
            $35 = 20-15+6-1+2\cdot10+5$ which holds.

            Element $d_{8,1}$, choosen at random, is the combination

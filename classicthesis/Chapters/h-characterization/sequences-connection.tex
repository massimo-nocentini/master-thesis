...
\marginpar{$y^{\alpha}$ and $\hat{h}(y)^{\beta}$ acts as rulers: \\
    if an element $d_{nk}$ is combined,\\
    the former scrolls (leftward) to column $k-\alpha$,\\
    the latter scrolls (upward) to row $n-\beta$}
...
\marginpar{$y^{1}$ and $\hat{h}(y)^{1}$: if $d_{nk}$ is combined then 
    scroll rulers to column $k-1$ and to row $n-1$, after 
    start combining as $\Gamma$ says\ldots}
\begin{displaymath}
...
\end{displaymath}
\\\\
\marginpar{an example involving $\mathcal{D}$}
...
\marginpar{which should not depend on $k$}
\begin{lenghtydisplaymath}
    \left.\left[\Gamma(y)=

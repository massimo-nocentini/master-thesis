...
\marginpar{closed formulae for Catalan coefficient $c_{j}$ and $d_{nk}\in\mathcal{C}$}
On the other hand, the generic element $d_{nk}\in\mathcal{C}$ admits 
closed formulae too. According to the article \emph{Identities induced by Riordan arrays}, two 
of them are of particular interest in order to complete our proofs:
\begin{equation}
...
column $\vect{c}_{k}$ of $\mathcal{C}$. Let $S$ be a \emph{recursively enumerable} 
set of natural numbers and $k\in\mathbb{N}$, then
$\diagup_{S}^{k}$ denotes a \emph{segment} of column $\vect{c}_{k}$ according
to the following relation:
\begin{equation}
    \begin{split}
    \diagup_{\lbrace\alpha_{0},\alpha_{1},\alpha_{2},\ldots\rbrace}^{k}&=
        \left(\vect{c}_{k},\lbrace k+\alpha_{0},k+\alpha_{1},k+\alpha_{2},\ldots\rbrace\right)\\
        &= \lbrace d_{k+\alpha_{0},k},d_{k+\alpha_{1},k},d_{k+\alpha_{2},k},\ldots\rbrace
    \end{split}
...
\end{displaymath}
\marginpar{For explanation of \emph{when} and \emph{how}
    use \ac{LIF} see \emph{PUT HERE THE REFERENCE TO
    ARTICLE BY MERLINI, VERRI, SPRUGNOLI}}
...
\begin{proof}
    ...
    Recall we can write it, according 
...
\input{Chapters/mod-p-characterization/catalan/alternating-row-include-figure.tex}
...
\begin{theorem}
    ...
    \end{displaymath}
    for some $s$ according segment definition in \autoref{eq:column:segment}.
\end{theorem}

\begin{proof}
Let $d_{s,2^{\alpha}-1}$ a coefficient in the segment $\Phi^{(\alpha)}$, 
according \autoref{eq:convolution:expansion:for:generic:element:in:catalan:array} 
write it as:
\begin{displaymath}
...
\begin{displaymath}
    \Omega^{(\alpha)}=\lbrace c_{2^{\alpha-(\alpha-1)}-1},c_{2^{\alpha-(\alpha-2)}-1},\ldots, 
        c_{2^{\alpha-1}-1},c_{2^{\alpha}-1}\rbrace
\end{displaymath}
where $\left|\Omega^{(\alpha)}\right|=\alpha$.
...
\input{Chapters/mod-p-characterization/catalan/dual-of-mirror-segment-include-figure.tex}
...
Another approach\footnote{REMOVE THIS PARAGRAPH SINCE IT IS ALREADY
DESCRIBED IN THE INTRODUCTORY SECTION FOR $\mathcal{C}$} 
is to find a closed form for a coefficient in $\mathcal{C}$,
...
\input{Chapters/mod-p-characterization/catalan/mirrored-clusters-include-figure.tex}
...
\input{Chapters/mod-p-characterization/catalan/principal-cluster-include-figure.tex}

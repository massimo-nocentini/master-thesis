...
\emph{syntatic context}.  One of such method is \mintinline{python}|__call__|,
which is sent to an object \mintinline{python}|obj| when it is used like a
function, for example: \mint{python}|obj(array=anArray,
partitioning=aPartitioning)| for some objects \mintinline{python}|anArray| and
\mintinline{python}|aPartitioning|. We have \emph{overridden} such a method in
...
All the above classes stricly depends on how the Riordan array has been
...
\emph{visitors}. It is interesting how the \emph{Visitor pattern} can be
generalized to a net of \emph{dispatching messages}, without limiting its
application to hierarchies denoting a recursive domain.

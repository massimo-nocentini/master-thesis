...
\marginpar{another generating function for the Catalan numbers}
\begin{displaymath}
...
\marginpar{yet another generating function for the Catalan numbers}
\begin{displaymath}
...
we begin with an attempt which applies the Lagrange Inversion Formula. 
...
\marginpar{For explanation of when and how
    about Lagrange Inversion Formula see \cite{merlini:lagrange:2006}}
The version of the Lagrange Inversion Formula we want to apply is the following one:
\begin{displaymath}
...
\end{displaymath}
\fi
Lagrange Inversion Formula's ``from right to left'' direction allow us to make 
the following definitions:
\begin{displaymath}
    \Phi(t)=\frac{1-\sqrt{1-4\,t}}{2}\qquad \alpha=k+1\qquad \beta=k-n
\end{displaymath}
Since $\Phi(0)=0$ we cannot apply Lagrange Inversion Formula directly, so start over again 
and observe that:
\begin{displaymath}

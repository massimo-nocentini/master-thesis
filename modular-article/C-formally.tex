...
$c_{n}$, the $n$-th Catalan number, satisfies the following relation:
\begin{displaymath}
    c_{n} = c_{n,0} = [t^{n}]d(t)= [t^{n}]\frac{1-\sqrt{1-4\,t}}{2\,t}
\end{displaymath}
...
respectively \cite{rogers:1977}. We can use them to inductively define every coefficient $d_{nk}$ in a Riordan array
as follows:
\begin{equation}
...
\begin{equation}
    h(t) = tA(h(t)) \quad d(t)=\frac{d_{00}}{1-tZ(h(t))}
\end{equation}
where $d_{00}$ is the very first element in the top left corner of $\mathcal{R}$.
...
each coefficient $c_{n}$ can be written as the following combination:
\begin{displaymath}
    c_{n} = \sum_{k=0}^{n-1}{d_{n-1,k}}
\end{displaymath}
